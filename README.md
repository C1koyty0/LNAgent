...
| 已有 | **Worldbook 世界观工作台**：`source → extract → apply` 文档录入流；source 变更后会清理旧 preview，并在重新提炼前禁止 apply |
...
- [x] [Worldbook 世界观文档录入与结构化](docs/features/worldbook-implementation-plan.md)（WK0–WK6 + WB2.1 已实现）
...
- [世界观文档录入与结构化设计](docs/features/worldbook-document-ingestion-design.md) — document-first、LLM 提炼、按 scope 注入；WK0–WK6 + WB2.1 已实现
- [世界观文档录入与结构化实施计划](docs/features/worldbook-implementation-plan.md) — WK0–WK6 + WB2.1 已实现，含 source 变更后 preview 清理与 apply 保护
