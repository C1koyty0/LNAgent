...
### 风险 4：SSE 生命周期与 HTTP 连接强绑定

若后端直接在响应迭代器中同步消费 `model.stream()`，则浏览器中断连接时，生成器可能提前关闭，导致：

- `last_candidate` 只保存部分 token
- 前端刷新后只能看到半截候选正文
- 流式体验与会话最终状态不一致

**应对**：将流式生成与 HTTP 输出解耦；后端在独立执行单元中持续消费模型流，并在完成后写回完整 session 状态。
...
- [x] 前端中断 SSE 连接后，后端仍继续完成本轮生成并写回完整 `last_candidate`
- [x] `python -m unittest` 通过
...
- 讨论 / 写作模式切换（前端单按钮 toggle，先不改后端协议）
- WebSocket / 流式输出
- 鉴权与用户隔离
...
| 日期 | 说明 |
|------|------|
| 2026-06-09 | W7 收口：SSE 在前端断连后仍继续完成后端生成并写回完整 candidate；讨论/写作模式切换记为后续 TODO |
