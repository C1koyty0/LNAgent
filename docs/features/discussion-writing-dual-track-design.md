...
- `todo_items`：待写事项
- `constraints`：当前 scene 约束
- `open_questions`：可选，保留尚未定论点
...
GET  /api/projects/{project_id}/discussion
POST /api/projects/{project_id}/discussion/brief/refresh
...
- `discussion`：读取 raw chat + brief
- `brief/refresh`：显式刷新 brief（前端也可手动触发）
