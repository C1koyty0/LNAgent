...
class WebAppIntegrationTest(unittest.TestCase):
    ...
    def test_home_page_and_project_page_render(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("worldbook-panel", project_html)
            self.assertIn("worldbook-source", project_html)
            self.assertIn("worldbook-status", project_html)
            self.assertIn("worldbook-preview", project_html)
            self.assertIn("worldbook-preview-overview", project_html)
            self.assertIn("worldbook-preview-global-rules", project_html)
            self.assertIn("worldbook-preview-scopes", project_html)
            self.assertIn("worldbook-preview-glossary", project_html)
            self.assertIn("worldbook-preview-open-questions", project_html)
            self.assertIn("data-action='worldbook-save-source'", project_html)
            self.assertIn("data-action='worldbook-extract'", project_html)
            self.assertIn("data-action='worldbook-apply'", project_html)
            self.assertIn("worldbook-apply-button", project_html)
    ...
    def test_static_assets_are_served(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(b"worldbook-panel", css.body)
            self.assertIn(b"worldbook-status-badge", css.body)
            self.assertIn(b"worldbook-preview", css.body)
            self.assertIn(b"worldbook-preview-grid", css.body)
            ...
            self.assertIn(b"/worldbook", js.body)
            self.assertIn(b"worldbook-source", js.body)
            self.assertIn(b"worldbook-save-source", js.body)
            self.assertIn(b"worldbook-extract", js.body)
            self.assertIn(b"worldbook-apply", js.body)
            self.assertIn(b"worldbook-apply-button", js.body)
            self.assertIn(b"syncWorldbookActionButtons", js.body)
            self.assertIn(b'"preview_ready"', js.body)
            self.assertIn(b"renderWorldbook", js.body)
            self.assertIn(b"saveWorldbookSource", js.body)
            self.assertIn(b"extractWorldbook", js.body)
            self.assertIn(b"applyWorldbook", js.body)
            ...
            self.assertIn(b"renderWorldbookStatus", render_js.body)
            self.assertIn(b"renderWorldbookPreview", render_js.body)
            self.assertIn(b"renderWorldbookScopes", render_js.body)
            self.assertIn(b"renderWorldbookGlossary", render_js.body)
            self.assertIn(b"worldbook-status-badge", render_js.body)
            self.assertIn("重新提炼以生成最新 preview".encode("utf-8"), render_js.body)
